...
return [
    ...
    |--------------------------------------------------------------------------
    |
    | Следующие строки языка содержат сообщения об ошибках по умолчанию, используемые классом валидатора.
    | Некоторые из этих правил имеют несколько версий, такие как правила для размера. Вы можете настроить каждое из этих сообщений здесь.
    |
    */
    ...
    'missing_with' => 'Поле :attribute должно отсутствовать, если :values присутствуют.',
    'missing_with_all' => 'Поле :attribute должно отсутствовать, если присутствуют все значения из :values.',
    ...
    'required_with_all' => 'Поле :attribute обязательно, если присутствуют все значения из :values.',
    'required_without' => 'Поле :attribute обязательно, если отсутствуют :values.',
    'required_without_all' => 'Поле :attribute обязательно, если отсутствуют все значения из :values.',
    ...
    'verify_sms_code' => 'Вы ввели неверный код из SMS.',
    ...
    |--------------------------------------------------------------------------
    |
    | Здесь вы можете указать пользовательские сообщения о валидации для атрибутов, используя соглашение "attribute.rule",
    | чтобы назвать строки. Это позволяет быстро указать конкретное пользовательское сообщение о языке для заданного правила атрибута.
    |
    */
    ...
    'custom' => [
        ...
        'phone' => [
            'unique' => 'Аккаунт с таким номером уже существует',
        ],
        'password' => [
            'regex' => 'Пароль должен содержать как минимум 8 символов, цифры и букву'
        ]
    ],
    ...
    |--------------------------------------------------------------------------
    |
    | Следующие строки языка используются для замены атрибута заполнителя на более читаемое имя, такое как "Адрес электронной почты"
    | вместо "email". Это помогает нам сделать наше сообщение более выразительным.
    |
    */
